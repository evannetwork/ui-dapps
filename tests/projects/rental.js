...
const rentalFuncs = {
  ...
  openDigitalTwin: (browser, dtAddress) => {
    return browser
      .evan.waitForElementVisible('lindig-dashboard-component', wait.element)
...
const createTransportOrder = function (browser) {
  ...
  browser
    ...
    .click(`${ elements.dt.ordersContainer } .content-heading button.button-md`)
      .evan.waitForElementVisible('ion-popover ion-list ion-item:first-child', wait.element)
      .click('ion-popover ion-list ion-item:first-child')

    // check if transport order is loading and set values
    .evan.waitForElementVisible(`${ elements.order.create.form } input[placeholder="Geben Sie den Titel des Auftrags"]`, wait.element)
      ...
      .click(`${ elements.order.create.form } ion-datetime button`)
        .evan.waitForElementVisible('ion-picker-cmp .picker-toolbar div:nth-child(2) button', wait.element)
    ...
    .evan.syncFinished()
      ...
      .click(`.refresh-icon`)
      .evan.waitForElementVisible(`${ elements.dt.ordersContainer } ${ elements.dt.newOrder } td:first-child`, wait.loading)
...
const solveTransportOrder = function(browser) {
  ...
  browser
    ...
    .click(`${ elements.dt.ordersContainer } ${ elements.dt.newOrder } button`)
      .evan.waitForElementVisible(`${ elements.order.detail.metadata } ion-grid ion-col:nth-child(2) button:nth-child(2)`, wait.element)
    ...
    // finish 1. task
    .evan.waitForElementVisible(`${ elements.order.detail.todo(2) } .todo-header-container button`, wait.element)
      // activate todo
    ...
    // finish 2. task
    .evan.waitForElementVisible(`${ elements.order.detail.todo(3) } .todo-header-container button`)
      // activate todo
    ...
    .evan.waitForElementVisible(`${ elements.order.detail.todo(4) } .todo-header-container button`, wait.element)
      ...
      .click(`${ elements.order.detail.todo(4) } .todo-data > ion-item .picture-container > button`)
        .evan.waitForElementVisible(`snapshot-dialog .button-container button:nth-child(2)`, wait.element)
        .pause(1000)
    ...
    // finish 4. task
    .evan.waitForElementVisible(`${ elements.order.detail.todo(5) } .todo-header-container button`, wait.element)
      // open todo
      .click(`${ elements.order.detail.todo(5) } .todo-header-container button`)
      .evan.waitForElementVisible(`${ elements.order.detail.todo(5) } .todo-data input`, wait.element)
    ...
    // finish the task
    .evan.waitForElementVisible(`${ elements.order.detail.tasks } > div > button`, wait.element)
