...
<template>
  ...
  >
    ...
    <div class="flex-center text-primary" v-else-if="stripe.success">
      <div><evan-success /></div>
      <div>{{_profile.wallet.buy-eve.success | translate}}</div>
    </div>
    <div class="flex-center text-red" v-else-if="!!stripe.payError">
      <div><evan-failed /></div>
      <div>{{ `_profile.wallet.buy-eve.${ stripe.payError }` | translate }}</div>
    </div>
