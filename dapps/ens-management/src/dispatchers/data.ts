    ...
    new QueueSequence(
      ...
      async (service: ENSManagementService, queueEntry: any) => {
        ...
        for (let entry of queueEntry.data) {
          ...
          // save the address
          if (entry.originalData.owner !== entry.ensData.owner ||
              entry.originalData.address !== entry.ensData.address) {
            service.nameResolver.setAddress(entry.ensAddress, entry.ensData.address, activeAccount,
              entry.ensData.owner);
          }
        }
      }
