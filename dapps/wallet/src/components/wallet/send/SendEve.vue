    ...
    >
      ...
      <template v-else>
        <evan-form
          :disabled="sending"
          :form="form"
          :i18n-scope="'_wallet.send-eve.form'"
          :only-form="true"
          :stacked="true"
        />
        ...
        >
          <template v-slot:header>
            <h5 class="modal-title">
              {{ `_wallet.send-eve.accept.title` | translate }}
            </h5>
          </template>
          <template v-slot:body>
            <span
              v-html="
                $t(`_wallet.send-eve.accept.description`, {
          ...
          <template v-slot:footer>
            <evan-button
              ...
              "
            >
              {{ '_wallet.send-eve.accept.send' | translate }}
            </evan-button>
          </template>
        </evan-modal>
      </template>
      ...
      >
        <evan-button
          ...
          :is-loading="sending"
          :label="'_wallet.send-eve.send' | translate"
          class="w-100"
          type="primary"
          @click="showModal"
        />
