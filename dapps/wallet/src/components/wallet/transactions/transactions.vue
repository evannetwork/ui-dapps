...
<template>
  <div class="profile-transactions">
    <h3 class="font-weight-bold">
      {{ '_wallet.transactions.title' | translate }}
    </h3>
    <span
      v-html="
        $t('_wallet.transactions.description', {
    ...
    <template v-else>
        ...
        >
          ...
          >
            <p class="mb-0 font-weight-semibold">
              {{ '_wallet.transactions.empty' | translate }}
            </p>
          </div>
        </template>
      </evan-base-list>
          ...
          <div class="d-flex align-items-center h-100 px-3">
            <template v-if="item.type === 'creditCharged'">
              ...
              <div>
                <p class="mb-0 font-weight-semibold">
                  {{
                    '_wallet.transactions.credit-charged' | translate
                  }}
                </p>
                <small class="text-muted">
                  {{ '_wallet.transactions.charged-at' | translate }}
              ...
              <div>
                <p class="mb-0 font-weight-semibold">
                  <template v-if="item.type === 'failedTransaction'">
                    {{
                      '_wallet.transactions.credit-failed-charge'
                        | translate
                    }}
                  </template>
                  <template v-else>
                    {{
                      '_wallet.transactions.credit-running-charge'
                        | translate
                    }}
                  </template>
                </p>
      ...
      <div class="text-center mt-3">
        <evan-button
          ...
          type="secondary"
          :label="'_wallet.transactions.load-more' | translate"
