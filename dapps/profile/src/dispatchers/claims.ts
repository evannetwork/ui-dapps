...
export const profileClaimsDispatcher = new QueueDispatcher(
  [
    new QueueSequence(
      '_profile.profile-claims-dispatcher.title',
      '_profile.profile-claims-dispatcher.description',
      async (service: ProfileService, queueEntry: any) => {
        ...
        for (let entry of queueEntry.data) {
          ...
          // load current data
          await profile.loadForAccount(profile.treeLabels.activeClaims);

          // if data is returned, we do not need to set the operation permissions
          const exists = await profile.ipld.getLinkedGraph(
            profile.trees[profile.treeLabels.activeClaims],
            profile.treeLabels.activeClaims
          );

          // save the data
          await profile.setActiveClaims(entry.claims);

          // if no active claims are set before, set them
          if (!exists) {
            await service.bcc.rightsAndRoles.setOperationPermission(
              ...
              profile.treeLabels.activeClaims,      // name of the object
          ...
          await profile.storeForAccount(profile.treeLabels.activeClaims);
        }
      }
    )
  ],
